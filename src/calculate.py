...
def calculate_global_data():
    ...
    with open("covid.txt", "w") as file:
        ...
        file.write(
            f"The country with the highest COVID death count is {deaths_country}, with a total death count of {deaths}, accounting for {percentage_death}% of all death.\n\n")
...
def calculate_usa_gender_data():
    ...
    unknown_percentage = int(
        round(unknown_all_age[0] / usa_all_age[0], 2) * 100)
    ...
    pneumonia_compare3 = int(
        round(female_compare[1] / female_compare[3], 2) * 100)
    influenza_compare3 = round(female_compare[2] / female_compare[3], 4) * 100

    with open("covid.txt", "a") as file:
        file.write("### USA Gender Data ###\n\n")
        file.write(
            f"In the US alone, men are dying at alarmingly higher rates than women, accounting for roughly {male_percentage}% of all deaths right now. Women only make up about {female_percentage}%.\n")
        file.write(
            f"In terms of age group between the genders, the overall most vulnerable group to COVID are ages between {age_group_all}.\n")
        file.write(
...
def calculate_countries_gender_data():
    ...
    male_percentage = int(
        round(male_female_deaths[0] / male_female_deaths[2], 2) * 100)
    ...
    with open("covid.txt", "a") as file:
        ...
        file.write(
            f"In terms of gender data, the highest COVID count providing this data is {country_max_deaths}, besides the United States. There, men account for {male_percentage}% of all COVID deaths so far with females accounting for only {female_percentage}%. That's quite alarming.\n")
        file.write(
            f"However, strangely enough, women make up the majority of the population at {female_pop_percentage}%, outnumbering men who make up only {male_pop_percentage}%\n\n")
        ...
        for i in max_death_data:
            try:
                file.write(
                    f"Age Group: {i[0]}, Female Deaths: {i[1]}, Male Deaths: {i[2]}, Male Death Percentage: {round(i[2] / (i[1] + i[2]), 2) * 100}%, Female Death Percentage: {round(i[1] / (i[1] + i[2]), 2) * 100}%\n")
